...
public class SpringBoxNodeParticle : NodeParticle
{
    public SpringBoxNodeParticle(SpringBox box, string id) : this(box, id, box.RandomXInsideBounds(), box.RandomYInsideBounds(), box.Is3D() ? box.RandomZInsideBounds() : 0)
    {
        this.box = box;
    }

    public SpringBoxNodeParticle(SpringBox box, string id, double x, double y, double z) : base(box, id, x, y, z)
    {
    }
    ...
    protected virtual void Gravity(Vector3 delta)
    {
        SpringBox box = (SpringBox)this.box;
        bool is3D = box.Is3D();
